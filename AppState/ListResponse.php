...
class Google_Service_AppState_ListResponse extends Google_Collection
{
  ...
  {
    foreach ($items as $i) {
      if (!$i instanceof Google_Service_AppState_GetResponse) {
        throw new InvalidArgumentException('First argument to setItems must be an array of Google_Service_AppState_GetResponse');
      }
    }
    $this->items = $items;
  }
