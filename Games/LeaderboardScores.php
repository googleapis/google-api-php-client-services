...
class Google_Service_Games_LeaderboardScores extends Google_Collection
{
  ...
  {
    foreach ($items as $i) {
      if (!$i instanceof Google_Service_Games_LeaderboardEntry) {
        throw new InvalidArgumentException('First argument to setItems must be an array of Google_Service_Games_LeaderboardEntry');
      }
    }
    $this->items = $items;
  }
