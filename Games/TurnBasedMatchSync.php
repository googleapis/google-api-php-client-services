...
class Google_Service_Games_TurnBasedMatchSync extends Google_Collection
{
  ...
  {
    foreach ($items as $i) {
      if (!$i instanceof Google_Service_Games_TurnBasedMatch) {
        throw new InvalidArgumentException('First argument to setItems must be an array of Google_Service_Games_TurnBasedMatch');
      }
    }
    $this->items = $items;
  }
